...
// Formatea segundos a mm:ss.000 o Lider
...
// Formatea un valor en segundos a mm:ss.000
...
export default function GapChart({
  ...
  laps,
  title,
}: Readonly<GapChartProps>) {
  const [chartData, setChartData] = useState<any[]>([]);
  const [hiddenLines, setHiddenLines] = useState<string[]>([]);
  const [highlightedLine, setHighlightedLine] = useState<string | null>(null);

  useEffect(() => {
    if (!data || data.length === 0 || !laps || laps.length <= 1) {
      setChartData([]);
      return;
    }

    // Transformar los datos del formato esperado por Recharts
    const transformedData = laps.slice(1).map((lap, index) => {
...
// Funcion auxiliar para obtener el color de cada piloto
