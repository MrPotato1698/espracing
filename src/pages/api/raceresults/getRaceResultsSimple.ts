	...
	async function loadData() {
		...
		try {
			...
			for (let i = 0; i < datos.length; i++) {
				const resultTable = getResultTableData(datos[i], points.Name, points, carData);
				...
				const resultFastestLap = getResultFastestLap(datos[i], points, carData, layoutLength);
				...
				tablaResultadosString += createHeaderResultRow(i+1);
			...
			if(datos.length > 1 && i < (datos.length-1)) {
