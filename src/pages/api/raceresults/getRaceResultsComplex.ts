...
function initializeScript() {
  async function loadData() {
    ...
    if (error || !resultSetData) {
      window.alert("No se ha encontrado la carrera seleccionada");
      throw new Error("No se encontraron datos de la carrera");
    }

    const datos: RaceData = JSON.parse(resultSetData?.race_data_1 as string);
    ...
    if (!isCircuitExists) {
      return alert(
        "No se ha encontrado el circuito de la carrera seleccionada: " +
          errorCircuitExists
      );
    }
    ...
    const chunkSize = 5; // Número de pilotos por chunk
    const drivers = dlaps;
