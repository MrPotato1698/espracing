...
export const POST: APIRoute = async ({ request }) => {
  ...
  try{
    ...
    const newOpenDate = new Date(now.getTime() - 1000 * 60 * 60 * 24); // Un día antes
    const newCloseDate = new Date(now); // Un día después
    ...
    const { data, error } = await supabase
      .functions.invoke('close-inscriptions', {body: { name: 'Functions' },});
    if (error) {
      return new Response(
        JSON.stringify({
          error: true,
